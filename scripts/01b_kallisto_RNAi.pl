...
my $read_path_triticum = '/nbi/Research-Groups/NBI/Cristobal-Uauy/PB_AFLF/RNAi_timecourse/raw_data/PKG_ENQ-789_RNAi_data_transfer/';

my $ref = "/nbi/group-data/ifs/NBI/Cristobal-Uauy/PB_AFLF/RefSeqv1.1_masked/IWGSC_v1.1_ALL_20170706_transcripts_masked_NAM.fasta";
my $index = "/nbi/group-data/ifs/NBI/Cristobal-Uauy/PB_AFLF/RefSeqv1.1_masked/IWGSC_v1.1_ALL_20170706_transcripts_masked_NAM_kallisto0.44.0_index";

# NB make index by kallisto index -i IWGSC_v1.1_ALL_20170706_transcripts_masked_NAM_kallisto0.44.0_index IWGSC_v1.1_ALL_20170706_transcripts_masked_NAM.fasta

my $output_dir = "$path/PB_AFLF/RefSeqv1.1_masked/RNAi_timecourse/kallisto_results";
...
my $RNAi_RNA_seq_paired_list = 'input_for_kallisto_RNAi.txt';
...
open (INPUT_FILE, "$RNAi_RNA_seq_paired_list") || die "couldn't open the input file $RNAi_RNA_seq_paired_list!";
...
my $dir2 = $array[7];
my $pair_4_R1 = $array[8];
my $pair_4_R2 = $array[9];
my $output = $array[10];


chdir("$read_path_triticum/$dir1") or die "couldn't move to specific read directory";
...
SLURM
 ...
 my $tmp_file = "$output_dir/tmp/kallisto.$output";
  ...
  print SLURM "\ncd $read_path_triticum/$dir1\n";

  print SLURM "source kallisto-0.44.0\n";
  print SLURM "source samtools-0.1.19\n";

  print SLURM "kallisto quant -i $index -o $output_dir/$output -t 8 --pseudobam $pair_1_R1 $pair_1_R2 $pair_2_R1 $pair_2_R2 $pair_3_R1 $pair_3_R2 ../$dir2/$pair_4_R1 ../$dir2/$pair_4_R2 | samtools view -Sb - > $output_dir/$output".".bam \n";
