# Aim is to look at gradient tool output in RNAi data and group genes into clusters which have the same expression pattern

## first for normalised

setwd("C:/Users/TXA013/3D Objects/Borril Lab/Gradient Tool/RNAi_Gradient Tool/")
out_dir <- "C:/Users/TXA013/3D Objects/Borril Lab/Gradient Tool/RNAi_Gradient Tool/"

data <- read.csv(file="out_RNAi_timecourse_tpm.csv")
head(data)
dim(data)
...
# there are 78 different patterns
write.csv(sorted_up_down_data,file="sorted_up_down_data_HC_only_RNAi.csv")
...
write.csv(file=paste0(out_dir,"Gradient Tool_number_per_pattern_RNAi.csv"), number_per_pattern)
