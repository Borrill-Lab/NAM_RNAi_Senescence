...
out_dir <- "C:/Users/TXA013/3D Objects/Borril Lab/Go enrichment term analysis/Go_Term_Enrichment_Analysis_RNAi"

base_dir <- "C:/Users/TXA013/3D Objects/Borril Lab/Go enrichment term analysis/Go_Term_Enrichment_Analysis_RNAi"

# load genes DE in ImpulseDE2 + gradient tool RNAi
RNAi_DE <- read.csv(file="C:/Users/TXA013/3D Objects/Borril Lab/ImpulseDE2/ImpulseDE2_RNAi/merged_impuseDE2_padj0.001_gradient_tool_RNAi.csv")
head(RNAi_DE)
dim(RNAi_DE)
# now keep only DE in gradient tool and impulseDE2
RNAi_DE <- RNAi_DE[!is.na(RNAi_DE$pattern) & !is.na(RNAi_DE$padj),]
dim(RNAi_DE)
head(RNAi_DE)
...
library(dplyr)
library(magrittr)
number_per_pattern <- RNAi_DE %>%
  group_by(pattern) %>%
  summarize(n= length(pattern))

write.csv(file=paste0(out_dir,"RNAi_number_per_pattern.csv"), number_per_pattern, row.names = F)
...
grouped_patterns <- read.csv(file=paste0(out_dir,"RNAi_number_per_pattern_grouped.csv"),)
head(grouped_patterns)

# add in grouped pattern info to RNAi_DE
RNAi_DE <- merge(RNAi_DE, grouped_patterns, by.x = "pattern", by.y="pattern", all.x=T)
head(RNAi_DE)
dim(RNAi_DE)
sum(complete.cases(RNAi_DE))# check all rows have data

# save WT_DE with grouped patterns
write.csv(file=paste0(out_dir,"RNAi_DE_with_grouped_pattern.csv"), RNAi_DE, row.names = F)
...
expr_genes <- read.csv("C:/Users/TXA013/3D Objects/Borril Lab/Gradient Tool/RNAi_Gradient Tool/RNAi_tpm_FLB_0.5tpm_input_gradient_tool.csv")
head(expr_genes)
...
for (i in (unique(RNAi_DE$grouped_pattern))){
  ...
  de.genes <- (RNAi_DE[RNAi_DE$grouped_pattern == i,"X"])
...
merged_GO_data_spread_final[1:5,68:74]

merged_GO_data_spread_final <- merged_GO_data_spread_final %>%
  dplyr::select(grouped_pattern,num_genes,representative_pattern,num_genes_in_representative_pattern,`1`:`68`)
...
write.csv(file=paste0(out_dir,"GO_enrichment_per_cluster_RNAi.csv"), merged_GO_data_spread_final, row.names = F)